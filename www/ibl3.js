...
(async function () {
	...
	document.getElementById("colorpick").onchange = () => {
		run_model();
		reset_canvas();
	};
})();
...
function extractcolor(hex) {
	const tokens = hex.match(/[0-9a-z]{2}/gi);
	return tokens.map((t) => parseInt(t, 16) / 255);
}

window.latent_vector = new Array(3).fill(0);

async function load_onnx(callback) {
	const session = await ort.InferenceSession.create(
		"assets/weights/colorwise_z3.onnx"
	...
	async function execute(vector) {
		...
		const tensor = new ort.Tensor("float32", typed, [3, 3]);
	...
	async function run_model() {
		...
		const [rw, gw, bw] = extractcolor(colorpick.value);
		...
		for (let i = 0; i < A; i++) {
			for (let j = 0; j < B; j++) {
				for (let k = 0; k < C; k++) {
					const temp = M1.data[i * (B * C) + j * C + k];
	...
	for (let i = 0; i < 3; ++i) {
		...
		elm.setAttribute("min", "-5");
		elm.setAttribute("max", "5");
