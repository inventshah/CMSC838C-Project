...
// window.DIMS = 1;
window.latent_vector = new Array(window.DIMS * window.LATENT_SIZE).fill(0);

async function load_onnx(callback) {
	const session = await ort.InferenceSession.create(window.MODEL).catch((e) =>
		alert("the browser was unable to load the neural network")
	);

	async function execute(vector) {
		if (window.pin) {
			for (let i = 0; i < window.LATENT_SIZE; i++) {
				vector[window.LATENT_SIZE + i] = vector[i];
				vector[2 * window.LATENT_SIZE + i] = vector[i];
			}
		}
		const typed = Float32Array.from(vector);
		const tensor = new ort.Tensor("float32", typed, [
			window.DIMS,
			window.LATENT_SIZE,
		]); // 1 8
	...
	async function run_model() {
		const M1 = await execute(
			latent_vector.slice(0, window.latent_vector.length)
		);
	...
	const latentSliders = [];
	for (let i = 0; i < window.latent_vector.length; ++i) {
		const labelText = `P${i + 1}`;
		const elm = document.createElement("input");

		elm.setAttribute("type", "range");
		elm.setAttribute("min", `-${window.BOUND}`);
		elm.setAttribute("max", `${window.BOUND}`);
		elm.setAttribute("step", "0.01");
		elm.setAttribute("value", `${window.latent_vector[i]}`);
